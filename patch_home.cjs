...
// 1. Add states
const states_hook = `  const [tempoNamoro, setTempoNamoro] = useState({ anos: 0, meses: 0, dias: 0, horas: 0, minutos: 0, segundos: 0 })
...
const appdata_hook_orig = `      const data = snap.data()
      const lastReset = data.lastReset ? data.lastReset.toDate() : null`;

const appdata_hook_new = `      const data = snap.data()
...
const handlers_block = `  // Buscar Recados e Resposta da Pergunta
  useEffect(() => {
...
const potinho_old = `          <div className="bg-passion/5 p-6 rounded-lg border-2 border-dashed border-passion/20 text-center relative overflow-hidden">
            {!ideiaDate ? (
              <div className="space-y-4">
...
const potinho_new = `          <div className="bg-passion/5 p-6 rounded-lg border-2 border-dashed border-passion/20 text-center relative overflow-hidden flex flex-col items-center justify-center min-h-[150px]">
...
// 6. Remove 5 month modal
// find start index of "{/* Modal de Celebração" and end at next component "{/* --- CONTEÚDO PRINCIPAL --- */}"
const start_modal = "{/* Modal de Celebração de 4 Meses */}";
const end_modal = "{/* --- CONTEÚDO PRINCIPAL --- */}";

if (content.includes(start_modal) && content.includes(end_modal)) {
  const p1 = content.indexOf(start_modal);
  const p2 = content.indexOf(end_modal);
